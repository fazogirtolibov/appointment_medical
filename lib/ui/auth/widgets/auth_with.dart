    ...
    return ZoomTapAnimation(
      onTap: () async {
        ...
        if (title == "Google") {
          ...
          context.read<AuthBloc>().add(CreateUserWithGoogle(
              fcmToken: fcmToken ?? "", isCreate: isCreate));
        } else {
          // ignore: use_build_context_synchronously
          MySnackBar(context,
              notification: "Sign In with Facebook is not available",
              color: Colors.red,
              icon: const Icon(
                Icons.info,
                color: Colors.white,
